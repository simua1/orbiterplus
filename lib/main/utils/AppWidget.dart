...
import 'package:orbiterplus/integrations/utils/common.dart';
import 'package:orbiterplus/main.dart';
import 'package:orbiterplus/main/model/ListModels.dart';
...
Widget commonCacheImageWidget(String? url, double height, {double? width, BoxFit? fit}) {
  if (url.validate().startsWith('http')) {
    if (isMobile) {
      ...
      Widget thumb = PlaceHolderWidget(height: height, width: width,);
      try{
        thumb = Image.network(url!, height: height, width: width, fit: fit ?? BoxFit.cover);
      }
      catch(e){
        thumb = PlaceHolderWidget(height: height, width: width,);
        print(e.toString());
      }
...
Widget placeholderWidget() => Image.asset('images/placeholder.png', height: 200, fit: BoxFit.fitHeight);
